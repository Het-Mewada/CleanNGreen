...
export const blockUser = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    await logAdminAction({
      action: `${blockedUser.isBlocked ? "Blocked" : "Unblocked "} user ${
        blockedUser.name
      } (${blockedUser.email})`,
      performedBy: adminUsername,
    });
...
export const updateUser = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (user.name !== updatedData.name) {
      changeLog.push(
        `name changed from "${user.name}" to "${updatedData.name}"`
      );
    }
    ...
    if (user.email !== updatedData.email) {
      changeLog.push(
        `email changed from "${user.email}" to "${updatedData.email}"`
      );
    }
    ...
    if (user.role !== updatedData.role) {
      changeLog.push(
        `role changed from "${user.role}" to "${updatedData.role}"`
      );
    }
